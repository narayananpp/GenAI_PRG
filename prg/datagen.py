...
SAVE_FOLDER = "MDM_preds"


def compute_step_metrics(args):
    timestep, x0_hat = args
    if x0_hat.ndim == 2 and x0_hat.shape[0] == NUM_FEATURES and x0_hat.shape[1] != NUM_FEATURES:
        motion = x0_hat.T
    else:
        motion = x0_hat
    metrics = compute_physics_metrics(motion)
    return timestep, metrics


def generate_and_harvest_trajectories(model, diffusion, cond, num_frames, mean_ten, std_ten):
    B = next(iter(cond['y'].values())).shape[0] if isinstance(cond['y'], dict) else cond['y']['mask'].shape[0]
...
def save_sample(raw_name, traj_b, pool, save_dir):
    tasks   = [(t, x0) for t, x0 in traj_b.items()]
    results = pool.map(compute_step_metrics, tasks)

    traj_metrics  = {t: m for t, m in results}
    final_metrics = traj_metrics[0]
    ...
    with open(save_path, "wb") as f:
        pickle.dump({
            ...
            "metrics_trajectory": traj_metrics,
            "final_metrics":      final_metrics,
        }, f)


def generate_reward_dataset(model, diffusion, data, num_samples, num_frames, save_dir):
    ...
    try:
        for batch_idx, (motion_gt, cond) in enumerate(tqdm(data, desc="Generating")):
            if samples_seen >= num_samples:
                break

            actual_bs = motion_gt.shape[0]
            lengths   = cond['y']['lengths']   # (B,) actual frame counts per sample
            ...
            for b in range(actual_bs):
                ...
                try:
                    save_sample(raw_name, traj_b, pool, save_dir)
                except Exception as e:
    ...
    print(f"Done. Saved {samples_seen} samples to {save_dir}")
